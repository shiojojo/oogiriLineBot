...
function buildLatestTopicPayload(config, lastSyncedAnswerId) {
  Logger.log('buildLatestTopicPayload: START');
  ...
  try {
    ...
    Logger.log('buildLatestTopicPayload: sheet found, processing...');
    ...
    const topicCreatedAt =
      answersDesc[0].submittedAt || new Date().toISOString();
    ...
    Logger.log('buildLatestTopicPayload ERROR: ' + error.message);
    throw error;
  }
}
...
function syncTextTopicAnswersToTsukkomi() {
  try {
    Logger.log('=== syncTextTopicAnswersToTsukkomi START ===');
    ...
    Logger.log('Last synced answer ID: ' + lastSynced);

    const result = buildLatestTopicPayload(config, lastSynced);
    if (!result) {
      Logger.log(
        'syncTextTopicAnswersToTsukkomi: no new topic answers to sync'
      );
      return;
    }

    Logger.log(
      'Payload built - answers count: ' +
        result.payload.answers.length +
        ', newest ID: ' +
    ...
    if (status >= 200 && status < 300) {
      if (result.newestAnswerId) {
        ...
        Logger.log('Updated last synced ID to: ' + result.newestAnswerId);
      }
      Logger.log('=== syncTextTopicAnswersToTsukkomi SUCCESS ===');
    } else {
      Logger.log(
        '=== syncTextTopicAnswersToTsukkomi FAILED with status ' +
          status +
          ' ==='
      );
    }
  } catch (error) {
    Logger.log('=== syncTextTopicAnswersToTsukkomi ERROR ===');
    Logger.log('Error type: ' + typeof error);
    Logger.log('Error message: ' + (error.message || String(error)));
    Logger.log('Error stack: ' + (error.stack || 'No stack trace'));
    Logger.log('Full error object: ' + JSON.stringify(error, null, 2));
    throw error; // Re-throw to see in GAS UI
  }
}
...
function cronSyncTextTopicsToTsukkomi() {
  try {
    Logger.log(
      'cronSyncTextTopicsToTsukkomi: Starting execution at ' +
        new Date().toISOString()
    );
    syncTextTopicAnswersToTsukkomi();
    Logger.log('cronSyncTextTopicsToTsukkomi: Completed execution');
  } catch (error) {
    Logger.log('cronSyncTextTopicsToTsukkomi: FATAL ERROR');
    Logger.log('Error: ' + error.toString());
    Logger.log('Stack: ' + (error.stack || 'No stack'));

    // Email notification removed - not needed for this use case

    throw error; // Re-throw so GAS shows the error
  }
}
